...
const WLPI = () => {
  ...
  useEffect(() => {
    ...
    window.addEventListener('wlpi:reset-filter', resetToRecentFilter);

    return () => {
      window.removeEventListener('wlpi:reset-filter', resetToRecentFilter);
    };
  }, []);
  ...
  const unviewedRecordsCount = transactions.filter(t => !viewedRecordIds.has(t.id) && needsAttention(t)).length;
